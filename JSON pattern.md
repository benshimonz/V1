...
|Mandatory| Name      | Type     | Description
|---------|-----------|----------| ------
|         | con       | {...}    | Constraint <br> _mandatory_ **op**: "eq"/"ne"/gt"/"ge" <br> _mandatory_ **expr**: string <br> ops "lt" and "le" are not used. To avoid ambiguity - either _in range_ [0 .. expr] or _in range_ [1 .. expr] should be used. <br> or <br> _mandatory_ **op**: "lt"/"le" - L3/L4 only, only if aggOp is not "distinct" <br> _mandatory_ **expr**: string <br> or <br> _mandatory_ **op**: "in set" <br> _mandatory_ **expr**: array [string] of size 2 or more <br> or <br> _mandatory_ **op**: "in range" <br> _mandatory_ **expr**: array [string] of size 2 <br> _mandatory_ **iType**: interval type: "()"/"(]"/"[)"/"[]"  <br> <br> Each element in **expr** may be a constant (e.g. "2"), a tag (e.g. "{2}") or a complex expression (e.g. "{2}+5"). It should be evaluated to a non-negative int.
