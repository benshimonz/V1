...
| +       | eName     | string | Display name of the entity (e.g. "Lior Kogan"). It is better to read the name from the database. This  was the display name when the pattern was stored
...
|         | con       | {...}  | Constraint <br> **Over any property type:** <br> _mandatory_ **op**: "empty"/"not empty" <br> <br> **Over ordinal properties / function-range** (integer types, floating-point types, date, datetime): <br> _mandatory_ **op**: "eq"/"ne"/gt"/"ge"/"lt"/"le" <br> _mandatory_ **expr**: string. type should match the property's type / f(property)'s type <br> or <br> _mandatory_ **op**: "in" (set) <br> _mandatory_ **expr**: array [string]. type should match the property's type / f(property)'s type <br> or <br> _mandatory_ **op**: "in" (range) <br> _mandatory_ **expr**: array [string] of size 2. type should match the property's type / f(property)'s type <br> _mandatory_ **inttype**: interval type: "()"/"(]"/"[)"/"[]" <br> <br> **Over  string properties / function-range:** <br> _mandatory_ **op**: "eq"/"ne"/"contains"/"not contains"/"starts with"/"not starts with"/"ends with"/"not ends with"/"match"/"not match"/"like"/"not like" <br> _mandatory_ **expr**: string <br> or <br> _mandatory_ **op**: "in set"/"not in set" <br> _mandatory_ **expr**: array [string]
