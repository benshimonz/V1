...
The V1 language, all the content of [https://github.com/LiorKogan/V1](https://github.com/LiorKogan/V1), as well as all the content of [http://v1.ms/](http://v1.ms/) are licensed under the [CC BY-NC-SA 4.0 licence](https://creativecommons.org/licenses/by-nc-sa/4.0/). For other uses please contact the author.
...
Visual query languages are very attractive. They have the potential to be more 'user friendly' in the sense that patterns may be constructed and understand much more quickly, with much less mental effort. A long-standing challenge is to design a query language that is generic, has large expressive power, and is highly receptive and productive. This is what V1 aims to be. 
...
Both freeze directions are acceptable. Therefore - a line (instead of an arrow) is used in the pattern.
...
Green rectangles and orange rectangles below a relationship can also be **chained**. When chained, each green/orange rectangle serves as a filtering step. The constraints are met only if there is an assignment that passes all filtering steps.
...
- An optional **constraint** on the value of _aggop(relprop) / aggop(relprop.f)_ of the relationships that satisfy the pattern - for each assignment combination to _S1_ entities. The constraint is in one of these forms:
  ...
  **L3 filters assignment combinations to _S1_ entities - to only those combinations for which the value of _aggop(relprop) / aggop(relprop.f)_ of the relationships that satisfy the pattern - satisfy the constraint.**
...
**M3 limits the assignment combinations to entities _S2_ - to the _n_ assignment combinations with the minimal / maximal value of _aggop(relprop) / aggop(relprop.f)_ of the assignments to the relationship on its top.**

If _S1_ is given:

**For each assignment combination to entities _S1_ - M3 limits the assignment combinations to entities _S2_ - to the _n_ assignment combinations with the minimal / maximal value of _aggop(relprop) / aggop(relprop.f)_ of the assignments to the relationship on its top.**
...
**R1 limits the assignments to the relationship - to the _n_ assignments with the smallest / largest value of _relprop / relprop.f_.**

If _S1_ is given:

**For each assignment combination to entities _S1_ - R1 limits the assignments to the relationship - to the _n_ assignments with the smallest / largest value of _relprop / relprop.f_.**
...
**R1:** For each property - only one property tag can be assigned. Similarly, for each (function, property) pair - only one property tag can be assigned to _f_(property).
...
**R3 - Tag scope:** For any quantifier except '&' - a tag defined in a branch cannot be referenced in other branches, and cannot be referenced left of the quantifier.
 -
...
**R16:** An aggregation tag {at} defined _per {e = ...}_ cannot be aggregated (_aggop {at}_) per _{e}_ or per supersets of _{e}_ 
